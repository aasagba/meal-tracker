...
app.service("MealService", function ($q, AuthService) {
	var self = {
		...
		'load': function () {
			...
			// Initialise Query
      var Meal = Parse.Object.extend("Meal"); // link to Meal table
      var mealQuery = new Parse.Query(Meal); // result is a query object
      ...
      mealQuery.find({
        success: function (results) {
          angular.forEach(results, function (item) {
            ...
            try {
              ...
              console.log("Error iterating meals: " + err);
            }
		...
		'track': function (data) {
			...
			var Meal = Parse.Object.extend("Meal"); // parse will create new table
      var user = AuthService.user;
      var file = data.picture ? new Parse.File("photo.jpg", {base64: data.picture}) : null;

      var meal = new Meal(); // row in table
      meal.set("owner", user);
